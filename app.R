...
#--------------------------------------------------------------------------------------------------------------------#
# a funtion to construct a dataframe 
...
# 
#     output$linePlot <- renderPlotly({
#         dfFunction<- function(a1,a2,b1,b2,sd1,sd2,size) {
#             x<- seq(-10,10,0.1)
#             y1<- x*b1+a1
#             y2<- 1/(1+exp(-a2-b2*x))
#             #data frame for the line plot
#             df1<-data.frame(x,y1,y2)
#             
#             random_x<- rnorm(size,mean = 0, 3)
#             random_num1<- rnorm(size, mean=0, sd=sd1)
#             random_num2<- rnorm(size, mean=0, sd=sd2)
#             
#             y1_data<- random_num1 + random_x*b1+a1
#             y2_data<- 1/(1+exp(random_num2-a2-b2 * random_x))
#             #data frame for the scatter plot
#             df2<-data.frame(random_x,y1_data,y2_data)
#             
#             #merging two data frames
...
#         }
#         
#         df<-reactiveValues(df=dfFunction(input$a1,input$a2,input$b1,input$b2,input$sd1,input$sd2,input$size))
# 
#         p<- plot_ly(df$df)%>%
#             add_trace(x=x,y=y1,type = 'scatter', mode = 'lines', name = "y1=a1+b1*x",
#                       line = list(color = 'rgb(255, 129, 10)'))
# 
#         
